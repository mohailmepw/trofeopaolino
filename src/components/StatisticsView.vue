    ...
    <div class="stats-panel">
      ...
      <div v-if="statistiche.migliorMarcatore?.length" class="ranking-list">
        <div
          ...
          :class="{ 'row-top': i < 3 }"
        >
          <div v-if="i < 3" class="row-accent" :style="{ background: rankColors[i] }" />

          <div class="rank-wrap">
            <span class="rank-num" :style="i < 3 ? { color: rankColors[i] } : {}">{{ i + 1 }}</span>
          </div>

          <!-- team dot: logo o iniziali -->
          <div
            class="team-dot"
            :style="getTeamLogo(p.squadra) ? {} : { background: getTeamColor(p.squadra) }"
            :title="p.squadra"
          >
            <img v-if="getTeamLogo(p.squadra)" :src="getTeamLogo(p.squadra)" class="dot-img" />
            <span v-else>{{ getInitials(p.squadra) }}</span>
          </div>
          ...
          <span class="row-val" :style="i < 3 ? { color: rankColors[i] } : {}">{{ p.gol }}</span>
        </div>
      </div>
    ...
    <div class="stats-panel">
      ...
      <div v-if="statistiche.cleanSheets?.length" class="ranking-list">
        ...
        >
          ...
          <span class="row-val row-val-blue" :style="i < 3 ? { color: rankColors[i] } : {}">{{ p.cleanSheets }}</span>
        </div>
      </div>
...
.panel-title { font-size: 12px; font-weight: 600; color: rgba(255,255,255,0.5); text-transform: uppercase; letter-spacing: 0.1em; }
...
.row-accent { position: absolute; left: 0; top: 0; bottom: 0; width: 3px; opacity: 0.8; }

.rank-wrap { width: 20px; flex-shrink: 0; text-align: center; }
.rank-num { font-size: 12px; font-weight: 700; color: rgba(255,255,255,0.25); }

/* ── team dot con supporto logo ────────────────────────────────────────── */
.team-dot {
...
.row-name { font-size: 13px; font-weight: 500; color: #fff; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.row-team { font-size: 11px; color: rgba(255,255,255,0.3); }

.bar-wrap { width: 60px; height: 4px; background: rgba(255,255,255,0.08); border-radius: 2px; overflow: hidden; flex-shrink: 0; }
.bar { height: 100%; border-radius: 2px; transition: width 0.6s ease; }

.row-val { font-size: 15px; font-weight: 700; color: #4ade80; min-width: 24px; text-align: right; flex-shrink: 0; }
.row-val-blue { color: #60a5fa; }

.empty-state { text-align: center; padding: 40px 20px; color: rgba(255,255,255,0.25); font-size: 13px; }
.empty-circle { width: 36px; height: 36px; border-radius: 50%; border: 2px solid rgba(74,222,128,0.2); margin: 0 auto 12px; }
.empty-circle-blue { border-color: rgba(96,165,250,0.2); }
